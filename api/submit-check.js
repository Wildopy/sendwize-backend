// ─────────────────────────────────────────────────────────────
// SENDWIZE — submit-check.js v5.0
//
// v5.0 — Campaign Defence
//   + defence-assemble: Pull evidence from existing tools, extract
//     claims via Claude, cross-ref relationships, snapshot sender
//   + defence-approve: Freeze evidence, snapshot URLs, start monitoring,
//     generate The Letter, emit compliance fixes from claims
//   + defence-add-event: Manual timeline entries with lifecycle transitions
//   + handleDossierGet returns all Defence fields
//   + handleDossierList returns defenceStatus + launchDate
//   + Exposure integration: extracted claims emit fixes to dashboard
//
// v4.32: Campaign monitoring (MonitoringActive, landing page URLs)
// v4.31: The Letter (clearance notice or simulated regulator letter)
// v4.30: atFetch retry. v4.29: re-verification stickiness.
// ─────────────────────────────────────────────────────────────
...
// ── Serialise module fields to readable text ──────────────────
function serialiseModuleFields(key, fields) {
  ...
  if (key === 'ConsentMechanism') {
    ...
    if (fields.softOptIn1) lines.push(`Soft opt-in — own similar products: Yes`);
    if (fields.softOptIn2) lines.push(`Soft opt-in — chance to opt out at collection: Yes`);
    if (fields.softOptIn3) lines.push(`Soft opt-in — opt-out in every message: Yes`);
    if (fields.softOptIn4) lines.push(`Soft opt-in — B2C contact: Yes`);
...
function buildGapSummary(moduleFields) {
  ...
  for (const key of DOSSIER_MODULES) {
    ...
    const req = REQUIRED_FIELDS[key] || [];
    const missing = req.filter(f => { const v = fields[f]; return v === undefined || v === null || v === '' || v === false; });
    if (missing.length > 0) gaps.push(`${key}: missing ${missing.join(', ')}`);
    if (key === 'ContentCheck') {
    ...
    if (key === 'Suppression' && fields.suppressionApplied === 'No — not yet applied') gaps.push('Suppression: suppression list not applied');
    if (key === 'ConsentMechanism' && fields.lawfulBasis === 'Legitimate interest (UK GDPR)' && !fields.liaSummary) gaps.push('ConsentMechanism: LI basis claimed but no LIA on record');
  }
  return gaps.join('\n');
}
...
async function generateLetter(moduleFields, evidenceStrength, campaignTitle, ownerName, dossierId, healthScore) {
  if (evidenceStrength === 'Strong') {
    return { type: 'clearance', lens: null, content: buildClearanceNotice(campaignTitle, ownerName, dossierId, healthScore), generatedAt: new Date().toISOString() };
  }
  ...
  try {
    ...
    const watermarked = content.startsWith('SIMULATION') ? content : 'SIMULATION — NOT ACTUAL REGULATOR CORRESPONDENCE\n\n' + content;
    return { type: 'letter', lens, content: watermarked, generatedAt: new Date().toISOString() };
  } catch (e) {
    console.error('generateLetter Claude call failed (non-fatal):', e);
    return { type: 'letter', lens, content: `SIMULATION — NOT ACTUAL REGULATOR CORRESPONDENCE\n\nWe were unable to generate a personalised letter at this time. Based on the gaps identified in your dossier, the ${lens.toUpperCase()} would be likely to request evidence addressing: ${buildGapSummary(moduleFields) || 'the incomplete fields in your compliance record'}.\n\nPlease review and complete all required fields in your dossier.`, generatedAt: new Date().toISOString(), fallback: true };
  }
}

// ── Monitored claim types extraction ──────────────────────────
function extractMonitoredClaimTypes(moduleFields) {
  const cc = moduleFields?.ContentCheck || {};
  const types = [];
  if (cc.pricingCompliant && !cc.pricingCompliant.includes('no pricing')) { types.push('reference_pricing'); types.push('drip_pricing'); }
...
function extractLandingPageUrls(moduleFields) {
  ...
  if (cc.referencePriceEvidence) { const m = cc.referencePriceEvidence.match(/https?:\/\/[^\s,;)]+/gi); if (m) urls.push(...m); }
  if (lp.collectionUrl && lp.collectionUrl.startsWith('http')) urls.push(lp.collectionUrl);
  if (si.unsubscribeUrl && si.unsubscribeUrl.startsWith('http')) urls.push(si.unsubscribeUrl);
  for (const mod of Object.values(moduleFields || {})) { if (mod?.notes) { const m = mod.notes.match(/https?:\/\/[^\s,;)]+/gi); if (m) urls.push(...m); } }
  return [...new Set(urls)];
}
...
function refineSeverity(fixType, emailVolume) {
  ...
  const rules = { invalid_consent_mechanism: 'critical', missing_unsubscribe: isLarge ? 'critical' : 'high', expired_consent: isLarge ? 'critical' : isMicro ? 'medium' : 'high', suppressed_contact: isLarge ? 'critical' : 'high', no_soft_optin: isLarge ? 'critical' : 'high', frequency_abuse: isLarge ? 'high' : 'medium', dark_pattern: isLarge ? 'critical' : 'high', misleading_pricing: 'high', misleading_claim: 'high', fake_urgency: 'medium', third_party_list: 'high' };
...
// ── Claim-to-fix type mapping (v5.0) ──────────────────────────
const CLAIM_FIX_MAP = {
...
// ══════════════════════════════════════════════════════════════
// CAMPAIGN DEFENCE ENDPOINTS (v5.0)
// ══════════════════════════════════════════════════════════════
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: `Analyse this marketing email copy and extract every specific claim that could have regulatory implications under UK law (ICO/PECR, ASA/CAP Code, CMA/DMCCA 2024).
...
async function snapshotUrl(url) {
  try {
    ...
    const r = await fetch(url, { headers: { 'User-Agent': 'Sendwize-ComplianceMonitor/1.0' }, signal: controller.signal, redirect: 'follow' });
    clearTimeout(timeout);
    if (!r.ok) return { url, status: 'error', error: `HTTP ${r.status}`, capturedAt: new Date().toISOString() };
    const html = await r.text();
    const visibleText = html.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 2000);
...
// ── Calculate defence exposure from extracted claims ───────────
function calculateDefenceExposure(claims) {
  let icoLow = 0, icoHigh = 0, asaCount = 0, cmaCount = 0;
  for (const claim of claims) {
    const cat = claim.exposureCategory || '';
    if (cat === 'ICO') { icoLow += 8000; icoHigh += 40000; }
    else if (cat === 'ASA') { asaCount++; }
    else if (cat === 'CMA') { cmaCount++; }
  }
  return { icoLow, icoHigh, icoMidpoint: Math.round((icoLow + icoHigh) / 2), asaCount, cmaCount, totalClaims: claims.length };
}

// ── DEFENCE-ASSEMBLE ──────────────────────────────────────────
async function handleDefenceAssemble(req, res) {
  const { userId, dossierId, campaignCopy, listName, connectedUrls, campaignTitle, ownerName } = req.body ?? {};
  ...
  if (!recordId) {
    const cr = await atFetch(`${base}/Campaign_Dossiers`, {
      ...
      body: JSON.stringify({ records: [{ fields: { UserID: userId, CampaignTitle: campaignTitle || 'Untitled Campaign', OwnerName: ownerName || '', Status: 'Draft', DossierSource: 'Campaign Defence', CreatedDate: today, DefenceStatus: 'building' } }] }),
    });
  ...
  if (campaignCopy?.trim()) {
    try {
      const checkRes = await fetch(`${APP_URL}/api/submit-check?action=check`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, copy: campaignCopy, source: 'Campaign Defence' }) });
  ...
  if (listName?.trim()) {
    try {
      const lr = await atFetch(`${base}/List_Intelligence_Checks?filterByFormula=AND({UserID}='${userId}',FIND('${listName.replace(/'/g, "\\'")}',{ListName}))&sort[0][field]=CheckDate&sort[0][direction]=desc&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } });
      if (lr.ok) { const checks = (await lr.json()).records || []; if (checks[0]) { listCheck = checks[0]; listSummary = { listName: listCheck.fields.ListName, checkDate: listCheck.fields.CheckDate, totalContacts: listCheck.fields.TotalContacts, gateVerdict: listCheck.fields.GateVerdict, consentHealthScore: listCheck.fields.ConsentHealthScore, suppressCount: listCheck.fields.SuppressCount, checkId: listCheck.id }; } }
    } catch (e) { console.error('List check lookup failed (non-fatal):', e); }
    try {
      const cr = await atFetch(`${base}/List_Intelligence_Certificates?filterByFormula=AND({UserID}='${userId}',FIND('${listName.replace(/'/g, "\\'")}',{ListName}))&sort[0][field]=IssuedDate&sort[0][direction]=desc&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } });
  ...
  if (campaignCopy?.trim()) {
    try {
      ...
      for (const r of (affiliates.records || [])) { const name = r.fields.AffiliateName || ''; if (name.length > 2 && copyLower.includes(name.toLowerCase())) linkedRelationships.push({ type: 'affiliate', recordId: r.id, name, consentVerified: !!r.fields.ConsentChainVerified, senderIdentityOk: r.fields.SenderIdentityCompliant === 'Verified' }); }
      for (const r of (partners.records || [])) { const name = r.fields.PartnerName || ''; if (name.length > 2 && copyLower.includes(name.toLowerCase())) linkedRelationships.push({ type: 'partner', recordId: r.id, name, article26Confirmed: r.fields.Article26Status === 'Confirmed', brandSafetyFlag: !!r.fields.BrandSafetyFlag }); }
  ...
  try {
    ...
    if (pr.ok) { const profile = (await pr.json()).records?.[0]?.fields; if (profile) senderSnapshot = { companyName: profile.CompanyName || '', fromEmail: profile.DefaultFromEmail || '', sector: profile.Sector || '', revenueBand: profile.RevenueBand || '' }; }
  } catch (e) { console.error('Profile lookup failed (non-fatal):', e); }

  // 7. Format URLs
  const urls = (Array.isArray(connectedUrls) ? connectedUrls : []).filter(u => u?.url?.startsWith('http')).map(u => ({ url: u.url, label: u.label || '', type: u.type || 'landing_page', addedAt: now }));

  // 8. Calculate exposure from claims
  const exposure = calculateDefenceExposure(claims);

  // 9. Timeline event
  const events = [{ type: 'defence_created', date: now, title: 'Defence record created', detail: `${claims.length} claim${claims.length !== 1 ? 's' : ''} extracted. ${linkedRelationships.length} relationship${linkedRelationships.length !== 1 ? 's' : ''} linked.${exposure.icoMidpoint ? ' ICO exposure: £' + exposure.icoLow.toLocaleString() + '–£' + exposure.icoHigh.toLocaleString() + '.' : ''}`, severity: 'info' }];
  ...
  const patchFields = {
    ...
    ListSummarySnapshot: listSummary ? JSON.stringify(listSummary) : '', UpdatedAt: now,
  };
  if (campaignTitle) patchFields.CampaignTitle = campaignTitle;
  if (ownerName) patchFields.OwnerName = ownerName;

  try { await atFetch(`${base}/Campaign_Dossiers/${recordId}`, { method: 'PATCH', headers: authH, body: JSON.stringify({ fields: patchFields }) }); }
  ...
  return res.json({ success: true, dossierId: recordId, defenceStatus: 'building', claimsExtracted: claims.length, aiCheckResult: aiCheckResult ? { issueCount: aiCheckResult.issues?.length || 0, score: aiCheckResult.score || null } : null, listSummary, linkedRelationships: linkedRelationships.length, connectedUrls: urls.length, exposure, events });
}
...
async function handleDefenceApprove(req, res) {
  ...
  if (!aiCheckFrozen && f.AICheckId) {
    try { const cr = await atFetch(`${base}/AI_Compliance_Checks/${f.AICheckId}`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (cr.ok) aiCheckFrozen = JSON.stringify((await cr.json()).fields || {}); } catch (e) {}
  }
  ...
  const allUrlsToSnapshot = [...connectedUrls.map(u => u.url || u), ...landingUrls.filter(u => typeof u === 'string')];
  const uniqueUrls = [...new Set(allUrlsToSnapshot)].slice(0, 10);
  const snapshots = [];
  for (const url of uniqueUrls) { snapshots.push(await snapshotUrl(url)); }
  ...
  events.push({ type: 'defence_approved', date: now, title: 'Defence approved', detail: `Approved by ${approvedBy || f.OwnerName || 'Campaign owner'}. ${snapshots.filter(s => s.status === 'ok').length} URL${snapshots.length !== 1 ? 's' : ''} snapshotted. Evidence strength: ${evidenceStrength}.`, severity: 'info' });
  ...
  const patchFields = {
    ...
    LandingPageUrls: JSON.stringify([...new Set([...(connectedUrls.map(u => u.url || u)), ...landingUrls])]),
    RefPriceAlertStage: '', ComplianceAlertsJson: f.ComplianceAlertsJson || '[]',
  };
  try { await atFetch(`${base}/Campaign_Dossiers/${dossierId}`, { method: 'PATCH', headers: authH, body: JSON.stringify({ fields: patchFields }) }); }
  ...
  for (const claim of claims) {
    ...
    if (mapping) {
      fetch(`${APP_URL}/api/generate-fix`, {
        ...
        body: JSON.stringify({ userId, fixType: mapping.fixType, description: `Campaign Defence: "${(claim.claim || '').slice(0, 150)}" — ${claim.ruleRef || 'requires evidence'}. Evidence needed: ${(claim.evidenceNeeded || '').slice(0, 200)}`, tool: 'Campaign Defence', severity: mapping.severity, sourceRecordId: dossierId }),
  ...
  return res.json({ success: true, dossierId, defenceStatus: 'approved', evidenceStrength, healthScore, urlsSnapshotted: snapshots.filter(s => s.status === 'ok').length, monitoredClaimTypes, letter, events });
}
...
async function handleDefenceAddEvent(req, res) {
  ...
  const ALLOWED_EVENTS = new Set(['campaign_launched', 'creative_amended', 'offer_expired', 'campaign_paused', 'campaign_resumed', 'campaign_takedown', 'url_added', 'url_removed', 'evidence_updated', 'manual_note']);
  if (!ALLOWED_EVENTS.has(eventType)) return res.status(400).json({ error: 'Invalid eventType' });
  ...
  const event = { type: eventType, date: now, title: title || eventType.replace(/_/g, ' '), detail: detail || '', severity: ['campaign_takedown', 'creative_amended'].includes(eventType) ? 'warning' : 'info' };
...
// ══════════════════════════════════════════════════════════════
// EXISTING DOSSIER ENDPOINTS (unchanged)
// ══════════════════════════════════════════════════════════════
...
async function handleDossierCreate(req, res) {
  ...
  const AIRTABLE_TOKEN = process.env.AIRTABLE_TOKEN; const BASE_ID = process.env.BASE_ID; const today = new Date().toISOString().split('T')[0];
  const fields = { UserID: userId, CampaignTitle: campaignTitle, OwnerName: ownerName || '', Status: 'Draft', DossierSource: dossierSource, CreatedDate: today };
  if (prefill && typeof prefill === 'object') {
    for (const key of DOSSIER_MODULES) { if (prefill[key]) fields[key] = typeof prefill[key] === 'string' ? prefill[key] : serialiseModuleFields(key, prefill[key]); }
    if (Object.keys(prefill).some(k => DOSSIER_MODULES.includes(k))) fields.ModuleFieldsJson = JSON.stringify(prefill);
  }
  const r = await atFetch(`https://api.airtable.com/v0/${BASE_ID}/Campaign_Dossiers`, { method: 'POST', headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ records: [{ fields }] }) });
  if (!r.ok) return res.status(r.status).json({ error: 'Failed to create dossier', detail: await r.text() });
  const record = (await r.json()).records?.[0]; const dossierId = record?.id;
  fetch(`${APP_URL}/api/profile?action=streak`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId }) }).catch(() => {});
...
async function handleDossierList(req, res) {
  ...
  if (!userId) return res.status(400).json({ error: 'Missing userId' });
  const AIRTABLE_TOKEN = process.env.AIRTABLE_TOKEN; const BASE_ID = process.env.BASE_ID;
  const r = await atFetch(`https://api.airtable.com/v0/${BASE_ID}/Campaign_Dossiers?filterByFormula={UserID}='${userId}'&sort[0][field]=UpdatedAt&sort[0][direction]=desc&maxRecords=${Math.min(parseInt(limit, 10) || 20, 100)}`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } });
  if (!r.ok) return res.status(r.status).json({ error: 'Failed to fetch dossiers' });
  const data = await r.json();
  const dossiers = (data.records || []).map(record => {
    const f = record.fields; const filled = DOSSIER_MODULES.filter(m => f[m]?.trim()).length; const pct = Math.round((filled / DOSSIER_MODULES.length) * 100);
    const verifiedRaw = f.LastVerified || f.SubmittedAt || '';
    let daysSinceVerified = null, needsReview = false;
    if (f.Status === 'Submitted' && verifiedRaw) { const vd = new Date(verifiedRaw); if (!isNaN(vd)) { daysSinceVerified = Math.floor((Date.now() - vd.getTime()) / 86400000); needsReview = daysSinceVerified > REVERIFY_DAYS; } }
    return { dossierId: record.id, campaignTitle: f.CampaignTitle || 'Untitled Campaign', ownerName: f.OwnerName || '', status: f.Status || 'Draft', dossierSource: f.DossierSource || 'Standalone', evidenceStrength: f.EvidenceStrength || null, healthScore: f.HealthScore || null, modulesComplete: filled, modulesPct: pct, createdDate: f.CreatedDate || '', updatedAt: f.UpdatedAt || '', submittedAt: f.SubmittedAt || '', lastVerified: f.LastVerified || '', daysSinceVerified, needsReview, defenceStatus: f.DefenceStatus || null, launchDate: f.LaunchDate || '' };
  });
...
async function handleDossierSave(req, res) {
  ...
  if (!userId || !dossierId || !module) return res.status(400).json({ error: 'Missing required fields' });
  if (!DOSSIER_MODULES.includes(module)) return res.status(400).json({ error: 'Invalid module' });
  const AIRTABLE_TOKEN = process.env.AIRTABLE_TOKEN; const BASE_ID = process.env.BASE_ID;
  const base = `https://api.airtable.com/v0/${BASE_ID}`; const authH = { Authorization: `Bearer ${AIRTABLE_TOKEN}`, 'Content-Type': 'application/json' };
  let moduleText = '', rawFields = moduleFields || null;
  if (moduleFields && typeof moduleFields === 'object') moduleText = serialiseModuleFields(module, moduleFields);
  else if (evidenceJson) { const ev = typeof evidenceJson === 'string' ? (() => { try { return JSON.parse(evidenceJson); } catch { return {}; } })() : (evidenceJson || {}); moduleText = ev[module] || ''; }
  const campaignTitle = moduleFields?.campaignTitle || null; const ownerName = moduleFields?.ownerName || null;
  let existing = null;
  try { const dr = await atFetch(`${base}/Campaign_Dossiers/${dossierId}`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (dr.ok) { const d = await dr.json(); if (d.fields?.UserID === userId) existing = d; } } catch {}
  if (!existing) { const lr = await atFetch(`${base}/Campaign_Dossiers?filterByFormula=AND({UserID}='${userId}',{CampaignID}='${dossierId}')&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); const ld = lr.ok ? await lr.json() : { records: [] }; existing = ld.records?.[0] || null; }
  const updateFields = { [module]: moduleText, UpdatedAt: new Date().toISOString() };
  if (campaignTitle) updateFields.CampaignTitle = campaignTitle; if (ownerName) updateFields.OwnerName = ownerName;
  if (rawFields) { let existingMFJ = {}; try { existingMFJ = JSON.parse(existing?.fields?.ModuleFieldsJson || '{}'); } catch {} existingMFJ[module] = rawFields; updateFields.ModuleFieldsJson = JSON.stringify(existingMFJ); }
  const recordId = existing?.id || dossierId;
  if (existing) { const r = await atFetch(`${base}/Campaign_Dossiers/${recordId}`, { method: 'PATCH', headers: authH, body: JSON.stringify({ fields: updateFields }) }); if (!r.ok) return res.status(r.status).json({ error: 'Failed to save module' }); }
  else { const createFields = { UserID: userId, CampaignID: dossierId, [module]: moduleText, DossierSource: 'Brief Checker' }; if (campaignTitle) createFields.CampaignTitle = campaignTitle; if (ownerName) createFields.OwnerName = ownerName; const r = await atFetch(`${base}/Campaign_Dossiers`, { method: 'POST', headers: authH, body: JSON.stringify({ records: [{ fields: createFields }] }) }); if (!r.ok) return res.status(r.status).json({ error: 'Failed to save module' }); }
...
async function handleDossierGet(req, res) {
  ...
  const base = `https://api.airtable.com/v0/${BASE_ID}`;
  let record = null;
  try { const dr = await atFetch(`${base}/Campaign_Dossiers/${dossierId}`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (dr.ok) { const d = await dr.json(); if (d.fields?.UserID === userId) record = d; } } catch {}
  if (!record) { const r = await atFetch(`${base}/Campaign_Dossiers?filterByFormula=AND({UserID}='${userId}',{CampaignID}='${dossierId}')&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (!r.ok) return res.status(r.status).json({ error: 'Failed to fetch dossier' }); record = (await r.json()).records?.[0] || null; }
  if (!record) return res.json({ dossierId, CampaignTitle: '', OwnerName: '', Status: 'Draft', DossierSource: 'Standalone', UserID: userId, ListProvenance: '', ConsentMechanism: '', ContentCheck: '', Suppression: '', SenderIdentity: '', moduleFields: {} });
  const f = record.fields;
  let moduleFields = {}; try { moduleFields = JSON.parse(f.ModuleFieldsJson || '{}'); } catch {}
  return res.json({
    ...
    ContentCheck: f.ContentCheck || '', Suppression: f.Suppression || '', SenderIdentity: f.SenderIdentity || '',
    CreatedDate: f.CreatedDate || '', UpdatedAt: f.UpdatedAt || '',
    SubmittedAt: f.SubmittedAt || '', LastVerified: f.LastVerified || '',
    MonitoredClaimTypes: f.MonitoredClaimTypes || '[]', ComplianceAlertsJson: f.ComplianceAlertsJson || '[]',
    LastComplianceCheck: f.LastComplianceCheck || '', MonitoringActive: f.MonitoringActive !== false,
    LandingPageUrls: f.LandingPageUrls || '[]', RefPriceAlertStage: f.RefPriceAlertStage || '',
    // Defence fields (v5.0)
    DefenceStatus: f.DefenceStatus || null, CampaignCopySnapshot: f.CampaignCopySnapshot || '',
    AICheckId: f.AICheckId || '', AICheckResultJson: f.AICheckResultJson || '',
    ListCheckId: f.ListCheckId || '', ListCertificateId: f.ListCertificateId || '',
    LinkedRelationships: f.LinkedRelationships || '[]', ClaimsExtracted: f.ClaimsExtracted || '[]',
    ConnectedUrls: f.ConnectedUrls || '[]', UrlSnapshots: f.UrlSnapshots || '[]',
    CampaignEventsJson: f.CampaignEventsJson || '[]',
    ApprovedAt: f.ApprovedAt || '', ApprovedBy: f.ApprovedBy || '',
    LaunchDate: f.LaunchDate || '', TakedownDate: f.TakedownDate || '',
    SenderIdentitySnapshot: f.SenderIdentitySnapshot || '', ListSummarySnapshot: f.ListSummarySnapshot || '',
    moduleFields, letter: f.LetterJson ? (() => { try { return JSON.parse(f.LetterJson); } catch { return null; } })() : null,
  });
}
...
async function handleDossierSubmit(req, res) {
  ...
  if (!userId || !dossierId) return res.status(400).json({ error: 'Missing userId or dossierId' });
  const AIRTABLE_TOKEN = process.env.AIRTABLE_TOKEN; const BASE_ID = process.env.BASE_ID;
  const base = `https://api.airtable.com/v0/${BASE_ID}`; const now = new Date().toISOString();
  let emailVolume = 'medium_send';
  try { const pr = await atFetch(`${base}/User_Profile?filterByFormula={UserID}='${userId}'&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (pr.ok) emailVolume = (await pr.json()).records?.[0]?.fields?.EmailVolume || 'medium_send'; } catch {}
  let currentRecord = null;
  try { const dr = await atFetch(`${base}/Campaign_Dossiers/${dossierId}`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (dr.ok) { const d = await dr.json(); if (d.fields?.UserID === userId) currentRecord = d; } } catch {}
  if (!currentRecord) { const lr = await atFetch(`${base}/Campaign_Dossiers?filterByFormula=AND({UserID}='${userId}',{CampaignID}='${dossierId}')&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (lr.ok) currentRecord = (await lr.json()).records?.[0] || null; }
  const f = currentRecord?.fields || {}; const actualRecordId = currentRecord?.id || dossierId;
  let moduleFields = {}; try { moduleFields = JSON.parse(f.ModuleFieldsJson || '{}'); } catch {}
  const snapshot = { snapshotAt: now, version: 1, modules: {} };
  for (const key of DOSSIER_MODULES) { snapshot.modules[key] = { text: f[key] || '', fields: moduleFields[key] || {} }; }
  let history = []; try { history = JSON.parse(f.VersionHistory || '[]'); } catch {} snapshot.version = history.length + 1; history.push(snapshot);
  const evidenceStrength = calculateOverallStrength(moduleFields); const healthScore = calculateHealthScore(moduleFields);
  const monitoredClaimTypes = extractMonitoredClaimTypes(moduleFields); const landingPageUrls = extractLandingPageUrls(moduleFields);
  const letterPromise = generateLetter(moduleFields, evidenceStrength, f.CampaignTitle || '', f.OwnerName || '', actualRecordId, healthScore);
  const issueList = Array.isArray(issues) ? issues : []; const fixResults = [];
  for (const issue of issueList) {
    const issueKey = typeof issue === 'string' ? issue : issue?.issue || ''; const mapping = ISSUE_TO_FIX[issueKey]; if (!mapping) continue;
    const finalSeverity = refineSeverity(mapping.fixType, emailVolume) || mapping.severity;
    try { const fixRes = await fetch(`${APP_URL}/api/generate-fix`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, fixType: mapping.fixType, description: mapping.description, tool: 'Campaign Dossier', severity: finalSeverity, volume: null, sourceRecordId: actualRecordId }) }); const fixData = await fixRes.json(); fixResults.push({ issue: issueKey, status: fixData.skipped ? 'duplicate_skipped' : 'created', fixId: fixData.fixId }); }
    catch (e) { console.error('generate-fix failed (non-fatal):', e); fixResults.push({ issue: issueKey, status: 'error' }); }
  }
  ...
  try {
    await atFetch(`${base}/Campaign_Dossiers/${actualRecordId}`, {
      ...
      body: JSON.stringify({ fields: { Status: 'Submitted', IssuesFound: issueList.length, SubmittedAt: now, LastVerified: now.split('T')[0], VersionHistory: JSON.stringify(history), EvidenceStrength: evidenceStrength, HealthScore: healthScore, MonitoredClaimTypes: JSON.stringify(monitoredClaimTypes), MonitoringActive: true, LandingPageUrls: JSON.stringify(landingPageUrls), RefPriceAlertStage: '', ComplianceAlertsJson: '[]' } }),
    });
  } catch (e) { console.error('Dossier status update failed (non-fatal):', e); }
  fetch(`${APP_URL}/api/profile?action=streak`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId }) }).catch(() => {});
  return res.json({ success: true, dossierId: actualRecordId, issuesFound: issueList.length, fixesGenerated: fixResults.filter(f => f.status === 'created').length, fixResults, evidenceStrength, healthScore, snapshotVersion: snapshot.version, monitoredClaimTypes, landingPageUrls, letter });
}

async function handleBriefCheck(req, res) {
  const { userId, campaignName, channel, audience, lawfulBasis, listSource, consentDate, coreOffer, listSize, suppressionDone, hasUnsubscribe, senderClear, softOptInAnswers, issues: frontendIssues, resultStatus: frontendStatus, dossierPrefill } = req.body ?? {};
  if (!userId) return res.status(400).json({ error: 'Missing userId' });
  const AIRTABLE_TOKEN = process.env.AIRTABLE_TOKEN; const BASE_ID = process.env.BASE_ID; const today = new Date().toISOString().split('T')[0];
  const issues = Array.isArray(frontendIssues) ? frontendIssues : []; const nonGreen = issues.filter(i => i.severity !== 'green');
  const redCount = issues.filter(i => i.severity === 'red').length; const amberCount = issues.filter(i => i.severity === 'amber').length; const greenCount = issues.filter(i => i.severity === 'green').length;
  const resultStatus = frontendStatus || (redCount > 0 ? 'Red' : amberCount > 0 ? 'Amber' : 'Green');
  let briefCheckId = null, totalExposureEstimate = 0;
  try {
    const briefRes = await atFetch(`https://api.airtable.com/v0/${BASE_ID}/Brief_Checks`, { method: 'POST', headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ records: [{ fields: { UserID: userId, CampaignName: campaignName || `Brief ${new Date().toLocaleDateString('en-GB')}`, CheckDate: today, BriefFieldsJson: JSON.stringify({ channel, audience, lawfulBasis, listSource, consentDate, coreOffer, suppressionDone, hasUnsubscribe, senderClear }), RedCount: redCount, AmberCount: amberCount, GreenCount: greenCount, IssuesJson: JSON.stringify(nonGreen), ResultStatus: resultStatus } }] }) });
  ...
  let emailVolume = 'medium_send';
  try { const pr = await atFetch(`https://api.airtable.com/v0/${BASE_ID}/User_Profile?filterByFormula={UserID}='${userId}'&maxRecords=1`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } }); if (pr.ok) emailVolume = (await pr.json()).records?.[0]?.fields?.EmailVolume || 'medium_send'; } catch {}
  for (const issue of nonGreen) {
    ...
    try { const fr = await fetch(`${APP_URL}/api/generate-fix`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, fixType: issue.fixType, description: `Brief Checker: ${issue.issue}. ${issue.description || ''}`.trim(), tool: 'Campaign Brief Checker', severity: finalSeverity, volume: null, sourceRecordId: briefCheckId }) }); const fd = await fr.json(); if (!fd.skipped) totalExposureEstimate += fd.exposureEstimate || 0; }
    catch (e) { console.error('generate-fix failed (non-fatal):', e); }
  }
  if (briefCheckId && totalExposureEstimate > 0) { atFetch(`https://api.airtable.com/v0/${BASE_ID}/Brief_Checks/${briefCheckId}`, { method: 'PATCH', headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ fields: { TotalExposureEstimate: totalExposureEstimate } }) }).catch(() => {}); }
  fetch(`${APP_URL}/api/profile?action=streak`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId }) }).catch(() => {});
