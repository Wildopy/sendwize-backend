...
// PascalCase — matches Airtable field names AND what the frontend sends
...
// ── Brief check — deterministic 8-field cross-reference ──────
...
// ─────────────────────────────────────────────────────────────
// dossier-save
// Saves evidence for a single module to Campaign_Dossiers.
// Module key is PascalCase e.g. "ListProvenance".
// evidenceJson may include CampaignTitle and OwnerName.
// ─────────────────────────────────────────────────────────────
async function handleDossierSave(req, res) {
  ...
  // Build the fields to write — module text goes into its named field,
  // title/owner update the top-level record fields
  const fields = {
    UserID:     userId,
    CampaignID: dossierId,
    [module]:   moduleText,           // e.g. ListProvenance: "our list was..."
...
// ─────────────────────────────────────────────────────────────
// dossier-get
// Returns the dossier record in a flat shape the frontend expects.
// Response fields: CampaignTitle, OwnerName, Status,
//   ListProvenance, ConsentMechanism, ContentCheck,
//   Suppression, SenderIdentity, UserID, CampaignID
// ─────────────────────────────────────────────────────────────
async function handleDossierGet(req, res) {
  ...
  if (!r.ok) {
    console.error('Campaign_Dossiers fetch failed:', r.status);
  ...
  if (!record) {
    // No record yet — return empty skeleton so frontend can render
    return res.json({
      CampaignTitle: '',
      OwnerName:     '',
      Status:        'Draft',
  ...
  // Return flat shape — all module fields directly on the object
  return res.json({
    recordId:          record.id,
...
// ─────────────────────────────────────────────────────────────
// dossier-submit
// Runs ISSUE_TO_FIX across submitted issues, fires generate-fix,
// marks dossier as Submitted.
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// brief-check
// Deterministic 8-field cross-reference.
// Generates fix records per issue.
// On non-Red result: creates Campaigns + Campaign_Dossiers skeleton,
// returns dossierId so frontend can redirect to /dossier?dossierId=xxx
// ─────────────────────────────────────────────────────────────
async function handleBriefCheck(req, res) {
  ...
  // Get emailVolume for refineSeverity
  let emailVolume = 'medium_send';
  try {
    const pr = await fetch(
      `https://api.airtable.com/v0/${BASE_ID}/User_Profile?filterByFormula={UserID}='${userId}'&maxRecords=1`,
  ...
  // On non-Red: create Campaigns record + Campaign_Dossiers skeleton
  let dossierId = null;
  if (resultStatus !== 'Red') {
    try {
