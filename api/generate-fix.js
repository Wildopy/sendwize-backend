...
export default async function handler(req, res) {
  ...
  try {
      ...
      try {
        ...
        if (dr.ok) {
          ...
          if (dd.records?.length > 0) {
            console.log(`Duplicate skipped: ${canonical} (original ${original})`);
    ...
    console.log(`Fix created: ${fixId} | canonical: ${canonical} | original: ${original} | category: ${def.category}`);
    return res.json({
