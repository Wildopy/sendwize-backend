...
function normaliseConsentBasis(raw) {
  if (!raw) return null;
  const lc = raw.toLowerCase().replace(/[-_ ]+/g, '_').trim();
  if (/^(direct_opt_in|explicit_consent|double_opt_in|confirmed_opt_in|opt_in|optin)$/.test(lc)) return 'direct_opt_in';
  if (/^(soft_opt_in|soft_optin|existing_customer|transactional)$/.test(lc)) return 'soft_opt_in';
  if (/^(legitimate_interest|li|legit_interest)$/.test(lc)) return 'legitimate_interest';
  if (/^(purchased|bought|rented|third_party_list|acquired)$/.test(lc)) return 'purchased';
  if (/^(partner|affiliate|shared|co_registration)$/.test(lc)) return 'partner';
  if (/^(mixed|multiple|various)$/.test(lc)) return 'mixed';
  return 'unknown';
}
  ...
  try {
    ...
    if (action === 'upload') {
      ...
      const rawRows = rows.map(row => {
        ...
        // v7.7: delivered/bounce count → rate
        if (c._deliveredCount != null && c.volume_sent) {
          c.delivery_rate = c._deliveredCount / c.volume_sent;
        }
      ...
      // v7.7 — consent basis compliance check
      const consentIssues = [];
      for (const [segName, segCamps] of Object.entries(segmentGroups)) {
        const bases = {}; segCamps.forEach(c => { if (c.consent_basis) bases[c.consent_basis] = (bases[c.consent_basis] || 0) + 1; });
        if (bases.purchased) consentIssues.push({ segment: segName, basis: 'purchased', severity: 'critical', message: `${segName}: ${bases.purchased} campaign(s) sent to a purchased list. PECR Reg 22 requires specific consent. ZMLUK fined £105,000 Dec 2025.` });
      ...
      for (const [segmentName, segCampaigns] of Object.entries(segmentGroups)) {
        ...
        // v7.7 — extended data
        const segRevenue = segCampaigns.reduce((s, c) => s + (c.revenue || 0), 0);
        const segBases = {}; segCampaigns.forEach(c => { if (c.consent_basis) segBases[c.consent_basis] = (segBases[c.consent_basis] || 0) + 1; });
        const segBounceRates = segCampaigns.map(c => c.bounce_rate).filter(r => r != null);
        const avgBounceRate = segBounceRates.length ? mean_arr(segBounceRates) : null;
        const segDeliveryRates = segCampaigns.map(c => c.delivery_rate).filter(r => r != null);
        const avgDeliveryRate = segDeliveryRates.length ? mean_arr(segDeliveryRates) : null;
        const channels = [...new Set(segCampaigns.map(c => c.channel).filter(Boolean))];
        ...
        savedSegments[segmentName] = {
          ...
          extendedData: {
            totalRevenue: segRevenue > 0 ? Math.round(segRevenue * 100) / 100 : null,
            consentBases: Object.keys(segBases).length ? segBases : null,
            avgBounceRate: avgBounceRate != null ? r4(avgBounceRate) : null,
            avgDeliveryRate: avgDeliveryRate != null ? r4(avgDeliveryRate) : null,
            channels: channels.length ? channels : null,
            consentIssues: consentIssues.filter(ci => ci.segment === segmentName),
          },
        };
      }
